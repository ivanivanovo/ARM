...
\ ------------------------------------------------------------------------------
REQUIRE #def  defs.f \ defs
REQUIRE mkdir sh.f   \ cmd
\ V-stack
REQUIRE NEW>S sstack.f \ S-stack 
\ ------------------------------------------------------------------------------
REQUIRE SAVE-VOCS vocstack.f \ Vocs-stack
\ ------------------------------------------------------------------------------
REQUIRE HEX[ bases.f \ bases
\ ------------------------------------------------------------------------------
...
: -Sym ( adr u sym-- adr' u') \ убрать из строки ведущие символы
    1>V 
    2DUP OVER + SWAP 
    ?DO I C@ 1V@ = IF 1 /STRING ELSE LEAVE THEN 
    LOOP 
    1V> DROP
    ;

: -BL ( adr u -- adr' u') \ убрать из строки ведущие пробелы
    BL -Sym ;
